...
class PredictionRequest(BaseModel):
    ...
    humidity_percent: float = Field(..., description="Relative humidity %", ge=0, le=100)
...
class PredictionResponse(BaseModel):
    ...
    outbreak_threshold: float = Field(..., description="Threshold for outbreak classification")
...
class ModelStatsResponse(BaseModel):
    ...
    performance_metrics: Optional[Dict[str, float]] = Field(None, description="Model metrics")
    data_source: Optional[str] = Field(None, description="Data source used for training")
